...
public class Scanners extends Product {
	...
	public String toString() {
		...
		productDetails+=String.format("%-15s%-17s%-21s%-15s%-10s%s\n", "Product ID", "Product Name", "Series", "Resolution", "Color", "Price");
		productDetails+=String.format("%-15s%-17s%-21s%-15s%-10s%s\n", "----------", "------------", "------", "----------", "-----", "-----");
		productDetails+=String.format("%-15s%-17s%-21s%s%-11s%-10s%.2f", super.getProductID(), super.getProductName(), series, resolution, "dpi", color, super.getPrice());
